      ...
      return new Promise((resolve, reject) => {
        try {
          ...
          if (!twitterWindow) {
            reject(
              new Error(
                'No se pudo abrir la ventana de Twitter. Verifica que no esté bloqueada por el navegador.'
              )
            );
            return;
          }
          ...
          let checkClosed = setInterval(async () => {
          ...
          // Establecer un tiempo límite (2 minutos) para evitar que la promesa quede pendiente indefinidamente
          setTimeout(() => {
            if (!twitterWindow.closed) {
              clearInterval(checkClosed);
              reject(
                new Error(
                  'Se agotó el tiempo de espera para compartir en Twitter'
                )
              );
            }
          }, 120000); // 2 minutos
          ...
          reject(new Error('No se pudo abrir la ventana de Twitter'));
        }
      });
    } catch (error) {
